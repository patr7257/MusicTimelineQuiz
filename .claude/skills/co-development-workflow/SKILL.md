...
description: Team co-development workflow for this repo: issue-first, board-synced, worktree-isolated. Use at the start of ANY task that changes files (feature, fix, chore, refactor, docs), when pointed at a GitHub issue or asked to "work on" or "pick up" something, before branching, and before opening a pull request. Coordinates multiple developers and parallel agent sessions against the GitHub Projects board (Backlog / Ready / In progress / In review / Done). Keywords: avoid double work, merge conflicts, who is working on what, claim an issue, kanban, board, parallel sessions, worktree.
---
